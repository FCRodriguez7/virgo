...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <!-- Main template controlling the order in which elements are handled. -->
  <xsl:template match="descmeta">
    ...
    <!--
      For the following elements, group multiple occurrences using only one
      label.
    -->
    <xsl:if test="culture">
      <dt>Culture</dt>
    ...
    <xsl:choose>
      <xsl:when test="@type">
        <xsl:choose>
          ...
          <xsl:otherwise>
            <dt>
              <titlecase>Place (<xsl:value-of select="$type"/>)</titlecase>
            </dt>
    ...
    <dt>
      <titlecase>
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:choose>
              <xsl:when test="self::time or self::covtime">
                date
              </xsl:when>
              <xsl:otherwise>
